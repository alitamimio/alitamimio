...
This does: it builds the dark card from fixed, invented figures, so the
result is identical from one night to the next whatever the real numbers do,
and compares its hash to the one recorded in tools/layout.sha256.
...
STATS = {"total": 500, "streak": 10, "best_streak": 20, "active_days": 75,
         "recent": [0, 1, 2, 3, 4], "through": "2000-01-01"}
LANGS = {"as_of": "2000-01-01",
         "bytes": {"TypeScript": 5, "Python": 4, "JavaScript": 3, "HTML": 2, "CSS": 1}}


def digest():
    """The card built from fixed figures, so only layout changes move it."""
    svg = build.build(build.DARK, STATS, LANGS, {})
